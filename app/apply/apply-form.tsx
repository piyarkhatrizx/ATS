...
export default function ApplyForm() {
  ...
  return <form onSubmit={submit} className="space-y-8">
    ...
    <fieldset className="border-0 p-0"><legend className="text-sm font-medium">Which opportunity describes you? (required)</legend><div className="mt-3 grid gap-2"><label className={`border px-4 py-3 text-sm ${form.caregivingInterest === "GENERAL_CAREGIVER" ? "border-[var(--accent)] bg-[var(--surface-selected)]" : "border-[var(--line)]"}`}><input className="mr-3 accent-[var(--accent)]" type="radio" name="caregivingInterest" value="GENERAL_CAREGIVER" checked={form.caregivingInterest === "GENERAL_CAREGIVER"} onChange={(event) => update("caregivingInterest", event.target.value)} required />I am looking for a general caregiver position</label><label className={`border px-4 py-3 text-sm ${form.caregivingInterest === "CURRENTLY_CARING_FOR_PATIENT" ? "border-[var(--accent)] bg-[var(--surface-selected)]" : "border-[var(--line)]"}`}><input className="mr-3 accent-[var(--accent)]" type="radio" name="caregivingInterest" value="CURRENTLY_CARING_FOR_PATIENT" checked={form.caregivingInterest === "CURRENTLY_CARING_FOR_PATIENT"} onChange={(event) => update("caregivingInterest", event.target.value)} />I am already caring for a patient</label></div></fieldset>
    <div className="grid gap-5 sm:grid-cols-2"><Input label="First name" name="firstName" required value={form.firstName} onChange={(event) => update("firstName", event.target.value)} /><Input label="Last name" name="lastName" required value={form.lastName} onChange={(event) => update("lastName", event.target.value)} /><Input label="Phone number" name="phone" required type="tel" value={form.phone} onChange={(event) => update("phone", event.target.value)} /><Input label="Email" name="email" required type="email" value={form.email} onChange={(event) => update("email", event.target.value)} /></div>
    {state === "error" && <p role="alert" className="text-sm text-[var(--danger)]">Please complete every field and try again.</p>}
