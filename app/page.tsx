  ...
  return (
    <main className="min-h-screen px-6 py-8 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-6xl">
        <header className="flex items-end justify-between border-b border-[var(--line)] pb-8">
          <div>
        ...
        <section className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {jobs.map((job) => (
            <Link key={job.id} href={`/jobs/${job.id}`} className="group border border-[var(--line)] bg-[#fbfaf6] p-6 transition-colors hover:border-[var(--accent)]">
              <div className="flex items-start justify-between gap-5">
                ...
                <span className={`text-[10px] font-semibold uppercase tracking-[0.16em] ${job.status === "OPEN" ? "text-emerald-700" : "text-[var(--ink-muted)]"}`}>{job.status.toLowerCase()}</span>
              </div>
          ...
          {!jobs.length && <p className="col-span-full border border-dashed border-[var(--line)] p-10 text-sm text-[var(--ink-muted)]">No jobs are configured yet. Add a job record to begin receiving resumes.</p>}
        </section>
      </div>
    </main>
  );
