...
#   npm run go
#
# Installs if needed, migrates, starts the dev server, waits until the app
...
if [ ! -f .env ]; then
  die "No .env file. Copy .env.example to .env and fill it in first."
fi

say "Applying migrations…"
if ! npm run --silent db:migrate >/dev/null 2>&1; then
  # Re-run visibly so the actual Prisma error reaches the user.
  npm run --silent db:migrate || true
  die "Migrations failed. Is DATABASE_URL reachable?"
fi
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  ...
  # Any HTTP response means Next is serving. A redirect to /login is the auth
  # gate working, not a failure, so 2xx/3xx/4xx all count as "up".
