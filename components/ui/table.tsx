...
export function Table({
  ...
  return (
    ...
    >
      <table
        className={`w-full border-collapse text-left text-[var(--text-sm)] ${className}`}
        {...props}
      >
        {children}
      </table>
...
export function TableHeader({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <thead
      className={`ui-sticky-head text-[var(--text-xs)] font-semibold uppercase tracking-[0.1em] text-[var(--ink-muted)] ${className}`}
    >
      {children}
    </thead>
  );
  ...
  return (
    <tr
      ...
      className={[
        ...
        active ? "shadow-[inset_2px_0_0_var(--accent)]" : "",
