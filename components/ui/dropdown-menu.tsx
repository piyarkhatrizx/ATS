...
const contentClass =
  "ui-menu z-50 min-w-44 border border-[var(--line)] bg-[var(--surface)] p-[var(--space-1)] text-[var(--text-sm)] shadow-[var(--shadow-panel)]";
...
export function DropdownMenuCheckboxItem({
  ...
  return (
    <RadixMenu.CheckboxItem className={`${itemClass} ${className}`} {...props}>
      <span aria-hidden="true" className="w-3 shrink-0 text-[var(--foreground)]">
...
export function DropdownMenuLabel({ children }: { children: ReactNode }) {
  return (
    <RadixMenu.Label className="px-[var(--space-2)] py-[var(--space-1)] text-[var(--text-xs)] font-semibold uppercase tracking-[0.1em] text-[var(--ink-muted)]">
      {children}
    </RadixMenu.Label>
  );
}
...
export function DropdownMenuShortcut({ children }: { children: ReactNode }) {
  return (
    <span className="ml-auto pl-[var(--space-4)] font-[family-name:var(--font-mono-ui)] text-[var(--text-xs)] text-[var(--ink-muted)]">
      {children}
    </span>
  );
}
