...
function Entry({ entry }: { entry: TimelineEntry }) {
  ...
  return (
    <li className="border-b border-[var(--line)] py-2 last:border-0">
      ...
      {row.body && (
        <p className="mt-1 whitespace-pre-wrap text-[var(--text-sm)] text-[var(--ink-muted)]">
          {row.body}
        </p>
      )}
      {(actor || job) && (
        <p className="mt-1 text-[var(--text-xs)] text-[var(--ink-muted)]">
          {actor ?? "System"}
          {job ? ` · ${job}` : ""}
        </p>
      )}
...
export function ActivityTimeline({
  ...
  if (!pinned.length && !entries.length) {
    return (
      <p className="border border-dashed border-[var(--line)] p-6 text-center text-[var(--text-sm)] text-[var(--ink-muted)]">
  ...
  return (
    <div className="space-y-5">
      {pinned.length > 0 && (
        <section aria-label="Pinned" className="border border-[var(--accent)] bg-[var(--surface)] px-3 py-1">
          <p className="pt-2 text-[var(--text-xs)] font-semibold uppercase tracking-[0.12em] text-[var(--accent-deep)]">
            Pinned
          </p>
          <ul>
            {pinned.map((entry) => (
      ...
      {groupByDay(entries).map((group) => (
        <section key={group.day}>
          <h3 className="sticky top-0 bg-[var(--background)] py-1 text-[var(--text-xs)] font-semibold uppercase tracking-[0.12em] text-[var(--ink-muted)]">
            {dayLabel(group.day)}
          </h3>
          <ul className="border border-[var(--line)] bg-[var(--surface)] px-3">
